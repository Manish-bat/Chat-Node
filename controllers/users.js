...
//       const findUserByEmail = await User.findOne({ 'auth.email': email });
//       if (findUserByEmail) {
//         return res
//           .status(400)
//           .json({ errors: { email: 'Email is already in use' } });
//       }
//       const findUserByName = await User.findOne({ 'auth.userName': userName });
//       if (findUserByName) {
//         return res
//           .status(400)
...
//       res.status(200).json(payload);
//     } catch (error) {
//       console.log('google set up reached but error');
//       console.log(error);
//     }
//   },
...
//   verifyPasswordChangedLink: async (req, res) => {
//     try {
...
//         });
//       }
//       if (findUser.config.method !== 'local') {
//         return res.status(401).json({
//           msg: 'Unauthorized',
//         });
//       }
//       const password = findUser.auth.local.password;
//       const verificationSecret = password + '-' + id;

//       const data = verifyToken(token, verificationSecret);
//       if (data.isErr) {
//         return res.status(401).json({
//           msg: 'Unauthorized / Timeout',
//         });
//       }
...
//           err: 'No url profile pic url found in request',
//         });
//       }
//       User.findByIdAndUpdate(
//         req.user._id,
...
//         }
//         return res.status(200).json({
//           msg: 'Deleted profile pic url',
//         });
//       });
//     } catch (error) {
//       console.log(error);
//       return res.status(500).json({ errTypr: 'Server error', error });
//     }
//   },
...
module.exports = {
  signup: async (req, res) => {
    try {
      ...
      if (newFoundUser) {
        return res.status(400).json({
          errors: {
            username: `Username ${username} already exists. Choose a new username`,
          },
        });
      }
  ...
  verifyPasswordChangeLink: async (req, res) => {
    try {
      ...
      if (!foundUser) return res.status(401).json({ msg: 'Not authorized.' });
  ...
  updateProfilePic: async (req, res) => {
    try {
      ...
      if (!url) {
        return res
          .status(400)
          .json({ err: 'No profile pic url found in request' });
      }
