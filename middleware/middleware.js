...
module.exports = {
  ...
  checkIfUserEmailExists: async (req, res, next) => {
    ...
    if (!existingUser)
      return res
        .status(401)
        .json({ message: 'The given email does not match up with any user.' });
    else req.user = existingUser;
    next();
  },
    ...
    if (req.user) {
      if (req.user.config.method !== 'local') {
        return res.status(401).json({
          message:
            'The given email is logged in via a social account. Please login through social login and not by email/password.',
        });
      }
    }
  ...
  checkIfUserInRoom: async (req, res, next) => {
    ...
    if (!room)
      return res
        .status(404)
        .json({ err: 'Room corresponding to the room id not found' });

    if (!room.people.includes(req.user.id)) {
      return res.status(401).json({
        err: 'You are not allowed to view the details of this group because you are not a member of the group',
      });
    }
    req.room = room;
    next();
